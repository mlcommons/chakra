...
class TraceLinker:
    ...
    def update_kineto_data(self, kineto_data: Dict) -> Tuple:
        ...
        kineto_tid_ops_map = kineto_data["kineto_tid_ops_map"]
    ...
    def process_dependent_gpu_ops(
        ...
        for gpu_op in sorted(dependent_gpu_ops, key=lambda x: x.timestamp):
            ...
            for sync_dep in gpu_op.sync_dep:
                if sync_dep.pytorch_op:
                    if "sync_dep_to" not in new_gpu_op:
                        new_gpu_op["sync_dep_to"] = []
                    if self.id_assigner.lookup_new_id(sync_dep.pytorch_op.id) not in new_gpu_op["sync_dep_to"]:
                        new_gpu_op["sync_dep_to"].append(self.id_assigner.lookup_new_id(sync_dep.pytorch_op.id))
